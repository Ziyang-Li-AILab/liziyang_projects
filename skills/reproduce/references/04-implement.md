...
- `configs/repro-default.yaml`: the hyperparam config matching `gaps_filled.md`
...
Each commit fills one gap and references its provenance. When the run fails or the result diverges, `git log` shows which gap might be wrong.

## What NOT to do

- Don't refactor the cloned code "to make it cleaner" before reproducing the result.
- Don't add type hints, docstrings, or tests to the cloned code unless they were missing in a way that broke imports.
- Don't fix the cloned code's bugs unless they prevent the run. The paper's result was produced by the code-as-written, including its bugs.
- Don't skip writing `train.py`. The reproduction is incomplete without an executable launch.
